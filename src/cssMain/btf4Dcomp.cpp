...
int main(int argc, char* argv[])
{
	...
	std::ostringstream strCoeffFileNameR;
	std::ostringstream strCoeffFileNameG;
	std::ostringstream strCoeffFileNameB;
	...
	strCoeffFileNameR << "Coeffs_BTF_" << strDataSetName << "_" << iSparsity << "_" << threshold << "_Y.mat";
	...
	if (!pTest->Save(strCoeffsDir, strCoeffFileNameR.str(), 200, true, dTimeDelta))	return false;
	std::cout << "Done in " << dTimeDelta << " seconds" << std::endl;

	//Calculate storage cost
	iCoeffCount = 0;
	iDictSize = 0;
	iCoeffCount = pTest->GetNNZ().sum();
	for (size_t i = 0; i < pDict->GetNumDict(); ++i)
		for (size_t j = 0; j < pDict->GetNumDictElems(i); ++j)
			iDictSize += pDict->GetDictElem(i, j).rows() * pDict->GetDictElem(i, j).cols();
	std::cout << "NNZ: " << iCoeffCount << std::endl;
	std::cout << "Size (MB): " << (iCoeffCount * 2 + iCoeffCount * 3 + iDictSize * 2 + pTest->GetM().rows() + pTest->GetNNZ().size()) / double(1024 * 1024) << std::endl;
	...
	iSparsity = 64;
	strCoeffFileNameG << "Coeffs_BTF_" << strDataSetName << "_" << iSparsity << "_" << threshold << "_U.mat";
	...
	std::cout << "Done in " << dTimeDelta << " seconds" << std::endl;
	std::cout << "Encoding and saving coefficients" << std::endl;
	if (!pTest->Save(strCoeffsDir, strCoeffFileNameG.str(), 200, true, dTimeDelta))	return false;
	std::cout << "Done in " << dTimeDelta << " seconds" << std::endl;
	...
	strCoeffFileNameB << "Coeffs_BTF_" << strDataSetName << "_" << iSparsity << "_" << threshold << "_V.mat";
	...
	std::cout << "Done in " << dTimeDelta << " seconds" << std::endl;
	std::cout << "Encoding and saving coefficients" << std::endl;
	if (!pTest->Save(strCoeffsDir, strCoeffFileNameB.str(), 200, true, dTimeDelta))	return false;
	std::cout << "Done in " << dTimeDelta << " seconds" << std::endl;
